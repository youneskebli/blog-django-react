...
class DashboardAPIViewTests(APITestCase):
    ...
    def test_dashboard_article_counts(self):
        ...
        Article.objects.create(title="Test Article", written_by=self.writer, created_at=timezone.now())
        Article.objects.create(title="Old Article", written_by=self.writer, created_at=thirty_days_ago - timezone.timedelta(days=1))
...
class ArticleCreateAPIViewTests(APITestCase):
    ...
    def test_create_article_authenticated(self):
        self.client.force_authenticate(user=self.writer.user)
        response = self.client.post(self.url, {'title': 'New Article', 'content': 'Content'})
...
class ArticleDetailAPIViewTests(APITestCase):
    ...
    def setUp(self):
        self.writer = Writer.objects.create(user=self.create_user(), name="Writer 1")
        self.editor = Writer.objects.create(user=self.create_user('editor'), name="Editor", is_editor=True)
        self.article = Article.objects.create(title="Test Article", written_by=self.writer)
...
class ArticleApprovalAPIViewTests(APITestCase):
    ...
    def setUp(self):
        self.editor = Writer.objects.create(user=self.create_user('editor'), name="Editor", is_editor=True)
        self.article = Article.objects.create(title="Test Article", status=Article.Status.PENDING)
...
class ArticlesEditedAPIViewTests(APITestCase):
    ...
    def test_get_edited_articles(self):
        self.client.force_authenticate(user=self.editor.user)
        Article.objects.create(title="Approved Article", edited_by=self.editor, status=Article.Status.APPROVED)
        Article.objects.create(title="Rejected Article", edited_by=self.editor, status=Article.Status.REJECTED)
